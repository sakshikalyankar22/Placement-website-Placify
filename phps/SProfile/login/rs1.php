...
$connect = mysqli_connect("localhost", "root", "amey_Hajare@#", "placement"); // Establishing Connection with Server
...
if ($USN1 && $password && $confirm) {
	...
	if ($password == $confirm) {
		...
		if ($USN2 == $USN1) {
			if ($sql = $connect->query("UPDATE `placement`.`slogin` SET `PASSWORD` ='$password' WHERE `slogin`.`USN` = '$USN1'")) {
